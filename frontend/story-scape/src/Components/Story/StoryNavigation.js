...
function StoryNavigation(props) {
  const { setSelectedTab, selected, story, setStory } = props;
  ...
  const [createChapter, { data }] = useMutation(CREATE_CHAPTER, {
    ...
    refetchQueries: [{ query: STORY_BY_ID, variables: { storyId: story.id } }],
    onQueryUpdated(observableQuery) {
      console.log(observableQuery);
    },
    variables: {
      input: { storyId: story.id, title: "CHAPTER TITLE", text: "" },
    },
  });
